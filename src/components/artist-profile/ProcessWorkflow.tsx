...
function StepCard({
  ...
  step: ArtistProcessStep;
  onClick: () => void;
}) {
  const hasModal = !!step.modalTitle;

  return (
    <button
      type="button"
      onClick={hasModal ? onClick : undefined}
      className={`flex flex-col items-center ${hasModal ? "cursor-pointer" : "cursor-default"}`}
      aria-haspopup={hasModal ? "dialog" : undefined}
    >
...
export function ProcessWorkflow({ steps }: ProcessWorkflowProps) {
  const [activeStep, setActiveStep] = useState<ArtistProcessStep | null>(null);
  ...
  return (
    <ProfilePanel className="px-4 py-7 sm:px-5 sm:py-8 md:px-6 md:py-10">
      <div className={`hidden lg:grid ${PROCESS_GRID} items-end`}>
        {steps.map((step, index) => (
          <Fragment key={step.label}>
            <StepCard step={step} onClick={() => setActiveStep(step)} />
      ...
      <ProcessStepModal step={activeStep} onClose={closeModal} />
    </ProfilePanel>
  );
}
